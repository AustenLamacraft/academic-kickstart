...
This is just the energy we found when we discussed the Stoner criterion in [Lecture 6]({{< ref "more-second-quantization" >}}). For the second order correction we need the matrix element $\braket{\mathbf{N}'}{H_\text{int}}{\mathbf{N}}$, which is nonzero if
